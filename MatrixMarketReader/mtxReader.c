...
void printMTX(struct Matrix_Market* mtx){
    ...
    for(INT_T i = 0; i < mtx->nnz; i++){
        ValueType* vals = (ValueType*)mtx->V; 
...
void printCSR(LONG_T* row_ptr, INT_T* col_idx, ValueType* values, int m){
    for(int r = 0; r < m; r++){
        for(LONG_T i = row_ptr[r]; i < row_ptr[r+1]; i++){
...
int main(){
    ...
    //     LONG_T *row_ptr = (LONG_T *)malloc((graph->m + 1) * sizeof(LONG_T));
    //     INT_T *col_idx = (INT_T *)malloc((graph->nnz) * sizeof(INT_T));
    //     ValueType *values = (ValueType *)malloc((graph->nnz) * sizeof(ValueType));
