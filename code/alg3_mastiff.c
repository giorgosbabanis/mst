...
{	
	...
	// Reading the grpah
		...
		int fd = open(LL_INPUT_GRAPH_PATH, O_RDONLY);
		struct stat st;
		size_t size = fstat(fd, &st);
		unsigned long* graph = (unsigned long *)mmap(NULL, st.st_size, PROT_READ, MAP_PRIVATE, fd, 0);

		wgraph = (struct ll_404_graph*)malloc(sizeof(struct ll_404_graph));

		wgraph->vertices_count = graph[0];
		wgraph->edges_count = graph[1];
		wgraph->offsets_list = &graph[2];
		wgraph->edges_list = (unsigned int*)&(wgraph->offsets_list[wgraph->vertices_count + 1]);
	...
	// 	else if(WEIGHTED_DATA_GRAPH)
	// 		// Reading the textual graph that do not require omp 
	// 		wgraph = get_ll_404_txt_graph(LL_INPUT_GRAPH_PATH, &read_flags);
